...
"""
TODO
add argument parser her for train_dev
"""
# parse first argument as train_dev
train_dev = "train"
if len(sys.argv) > 1:
    train_dev = sys.argv[1]

# setting the data paths
# train.json does not exist in spider data
# training data is the train_spider.json file
# For more info: README.txt under spider
...
# parse second argument as history_option (default: "full")
history_option = "full"
if len(sys.argv) > 2:
...
def get_table_dict(table_data_path):
    print("table_data_path = " + str(table_data_path))
    data = json.load(open(table_data_path))
    table = dict()
    for item in data:
        # print("item['db_id'] = " + str(item["db_id"]))
...
def parse_data(data):
    ...
    for item in data:
        ...
        # parser_item(item["question_toks"], item["sql"], table_dict[item["db_id"]], [], dataset)
            parser_item_with_long_history(item["question_toks"], 
                                            item["sql"], 
                                            table_dict[item["db_id"]], 
                                            [], 
                                            dataset)
        else:
            parser_item(item["question_toks"], 
                        item["sql"], 
                        table_dict[item["db_id"]], 
                        [], 
                        dataset)
    ...
    print("Train // Dev = {}".format(train_dev))
    for key in dataset:
