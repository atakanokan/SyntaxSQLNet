...
class SuperModel(nn.Module):
    ...
    def full_forward(self, q_seq, history, tables):
        ...
        # print("\nq_len:{}".format(q_len))
        # print("\nq_emb_var:{}".format(q_emb_var))

        col_seq = to_batch_tables(tables, B, self.table_type)
        col_emb_var, col_name_len, col_len = self.embed_layer.gen_col_batch(col_seq)
        # print("\ncol_seq:{}".format(col_seq))
        # print("\ncol_emb_var:{}".format(col_emb_var))
        # print("\ncol_name_len:{}".format(col_name_len))
        # print("\ncol_len:{}".format(col_len))
        ...
        # print("\nnot stack.isEmpty():{}".format(not stack.isEmpty()))
        ...
        while not stack.isEmpty():
            ...
            if isinstance(vet,tuple) and vet[0] == "root":
                ...
                score = self.key_word.forward(q_emb_var,q_len,
                                            hs_emb_var,hs_len,
                                            kw_emb_var,kw_len)
    ...
    def gen_sql(self, sql, table):
        ...
        # print("sql = " + str(sql))
        # {'sql': {'select': [('head', 'name', 8), 'none_agg'], 
        #           'where': [('head', 'name', 8), '=', 'terminal']}}
    ...
    def check_acc(self, pred_sql, gt_sql):
        """
        supermodel doesn't have accuracy metric because it is not used in training
        each module of supermodel should have this implemented.
        """
        pass
